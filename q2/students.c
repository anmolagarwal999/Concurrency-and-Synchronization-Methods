...
void seek_hospital(int id)
{
    ...
    while (true)
    {
        ...
        //Not necessary to lock but don't want a student to miss a hospital opportunity
        pthread_mutex_lock(&(stu_ptr[id]->mutex));
        stu_ptr[id]->curr_stat = 0; //student has started waiting
        pthread_mutex_unlock(&(stu_ptr[id]->mutex));

      
      
        printf(ANSI_GREEN "Student id %d   is waiting to be allocated a slot in a Vaccination Zone\n" ANSI_RESET, id);

        //Student hopes to get a vaccine
        ...
        {
            printf(ANSI_RED "Student %d has tested positive for antibodies \n He/She can now attend college\n" ANSI_RESET, id);
        ...
        {
            printf(BBLU "Student %d has tested negative for antibodies in his %d th attempt\n" ANSI_RESET, id, stu_ptr[id]->rounds_already);
        }
    ...
    tot_conclusions_left--;
    pthread_mutex_unlock(&conclusions_mutex);
    printf(BBLU "Student %d   is unable to develop anibodies even after 3 attempts,he has to return home\n" ANSI_RESET, id);
    return;
